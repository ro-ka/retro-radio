...
class MediaTransportSBC(MediaTransport):
  def initPipeline(self):
    ...
    jitterbuffer.set_property("latency", BUFF_LEN)
...
class Bluetooth:
  ...
  def start():
    ...
    if self.adapt is None:
      self.adapt = self.bluez.getAdapter(ADAPTER)

    if (not self.adapt or self.adapt is None):
      print("Adapter " + ADAPTER + " not found")
      return

    self.adapt.powerSet(True)
    self.adapt.discoverableSet(True)
    self.adapt.mediaEndpointRegisterSBC()
